/*
 ...
 Para luego almacenar la infoirmacion decodificada en un archivo temporal'
...
void sudo::verifying_password(){
    ...
    if(clave_decodificada == clave_admin and ban != true){
        //Si son iguales se le envia al atributo privado de key_clave un true
        setKey_clave(true);
    }else{
        //De lo contrario se le lleva un false
        setKey_clave(false);
    }
}
...
void sudo::verifying_existence(){
    ...
    if(contendido_archivo == ""){
        //Si el archivo de usuarios esta vacio, se envia fal a encontrado
        setEncontrado(false);
    }else{
        ...
        while(!Leer.eof()){
            ...
            for(int i = 0; i < 3; i++){
                char *puntero;
                if(i == 0){
                    //strtok recibe dos parametros, el primero es la linea que queremos dividir
